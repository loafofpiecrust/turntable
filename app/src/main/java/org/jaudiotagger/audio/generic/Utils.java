...
public class Utils
{
    ...
    public static String getExtension(DocumentFile f)
    {
        String name = f.getName().toLowerCase();
        int i = name.lastIndexOf(".");
        if (i == -1)
    ...
    */
    ...
    public static boolean rename(DocumentFile fromFile, DocumentFile toFile)
    {
        logger.log(Level.CONFIG,"Renaming From:"+fromFile.getUri() + " to "+toFile.getUri());
        ...
        //Rename File, could fail because being  used or because trying to rename over filesystems
        final boolean result = fromFile.renameTo(toFile.getName());
        if (!result)
        {
            // Might be trying to rename over filesystem, so try copy and delete instead
//            if (copy(fromFile, toFile))
