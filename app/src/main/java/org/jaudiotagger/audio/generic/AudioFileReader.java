...
public abstract class AudioFileReader
{
    ...
    */
    ...
    protected GenericAudioHeader getEncodingInfo(FileChannel raf) throws CannotReadException, IOException { return null; }
    ...
    protected Tag getTag(FileChannel raf) throws CannotReadException, IOException  { return null; }
    ...
    public AudioFile read(Context ctx, DocumentFile f)
            throws CannotReadException, IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
        if(logger.isLoggable(Level.CONFIG))
        {
            logger.config(ErrorMessage.GENERAL_READ.getMsg(f.getUri()));
        }

        if (!f.canRead())
        {
            throw new CannotReadException(ErrorMessage.GENERAL_READ_FAILED_FILE_TOO_SMALL.getMsg(f.getUri()));
        }
...
//        RandomAccessFile raf = null;
        ...
        try
        {
            input = (FileInputStream) ctx.getContentResolver().openInputStream(f.getUri());
            FileChannel chan = input.getChannel();

            if (chan.size() <= MINIMUM_SIZE_FOR_VALID_AUDIO_FILE)
            {
                throw new CannotReadException(ErrorMessage.GENERAL_READ_FAILED_FILE_TOO_SMALL.getMsg(f.getUri()));
            }

            GenericAudioHeader info = getEncodingInfo(chan);
            chan.position(0);
            Tag tag = getTag(chan);
            return new AudioFile(f, info, tag);
        ...
        {
            logger.log(Level.SEVERE, ErrorMessage.GENERAL_READ.getMsg(f.getUri()),e);
            throw new CannotReadException(f.getUri()+":" + e.getMessage(), e);
        }
        finally
        {
            try
            ...
            {
                logger.log(Level.WARNING, ErrorMessage.GENERAL_READ_FAILED_UNABLE_TO_CLOSE_RANDOM_ACCESS_FILE.getMsg(f.getUri()));
            }
        }
    }
}
