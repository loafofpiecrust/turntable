...
public abstract class AudioFileWriter
{
    ...
    protected void writeTag(Tag tag, FileChannel raf, FileChannel reader, FileChannel rafTemp) throws CannotWriteException, IOException {}
    ...
    private void precheckWriteDoc(AudioFile af) throws CannotWriteException
    {
        // Preliminary checks
        try
        {
        ...
        {
            throw new CannotWriteException(ErrorMessage.GENERAL_WRITE_FAILED
                    .getMsg(af.docFile.getUri()));
        }

        if (!af.docFile.canWrite() || !af.docFile.exists())
        {
            logger.severe(ErrorMessage.GENERAL_WRITE_FAILED.getMsg(af.docFile.getUri()));
            throw new CannotWriteException(ErrorMessage.GENERAL_WRITE_FAILED
                    .getMsg(af.docFile.getUri()));
        }

        if (af.docFile.length() <= MINIMUM_FILESIZE)
        {
            logger
                    .severe(ErrorMessage.GENERAL_WRITE_FAILED_BECAUSE_FILE_IS_TOO_SMALL
                            .getMsg(af.docFile.getUri()));
    ...
    public void write(Context ctx, AudioFile af) throws CannotWriteException
    {
//        logger.config("Started writing tag data for file:" + af.getFile().getName());
    ...
    public void writeDocFile(Context ctx, AudioFile af) throws CannotWriteException
    {
//        logger.config("Started writing tag data for file:" + af.getFile().getName());
        ...
        try
        {
            newFile = af.docFile.getParentFile().createFile(
                    af.docFile.getType(),
                    af.docFile.getName().replace('.', '_') + TEMP_FILENAME_SUFFIX
            );
//            newFile = File.createTempFile(af.docFile.getName().replace('.', '_'), TEMP_FILENAME_SUFFIX, af.getFile().getParentFile());
        }
        // Unable to create temporary file, can happen in Vista if have Create
        // Files/Write Data set to Deny
        catch (Exception ioe)
        {
            if(ioe.getMessage().equals(FILE_NAME_TOO_LONG) && (af.docFile.getName().length() > FILE_NAME_TOO_LONG_SAFE_LIMIT) )
            {
                try
                {
                    newFile = af.docFile.getParentFile().createFile(
                            af.docFile.getType(),
                            af.docFile.getName().substring(0,FILE_NAME_TOO_LONG_SAFE_LIMIT).replace('.', '_') + TEMP_FILENAME_SUFFIX
                    );
//                    newFile = File.createTempFile(af.getFile().getName().substring(0,FILE_NAME_TOO_LONG_SAFE_LIMIT).replace('.', '_'), TEMP_FILENAME_SUFFIX, af.getFile().getParentFile());
                ...
                catch (Exception ioe2)
                {
                    logger
                            .log(Level.SEVERE, ErrorMessage.GENERAL_WRITE_FAILED_TO_CREATE_TEMPORARY_FILE_IN_FOLDER
                                    .getMsg(af.docFile.getName(), af
                                            .docFile.getParentFile()
                                            .getUri()), ioe2);
                    throw new CannotWriteException(ErrorMessage.GENERAL_WRITE_FAILED_TO_CREATE_TEMPORARY_FILE_IN_FOLDER
                            .getMsg(af.docFile.getName(), af.docFile
                                    .getParentFile().getUri()));
                }
            }
            else
            {
        ...
        try
        {
//            rafTemp = new RandomAccessFile(newFile, WRITE_MODE).getChannel();
//            raf = new RandomAccessFile(af.getFile(), WRITE_MODE);
            rafTemp = ((FileOutputStream) ctx.getContentResolver().openOutputStream(newFile.getUri(), "rw")).getChannel();
            raf = ((FileOutputStream) ctx.getContentResolver().openOutputStream(af.docFile.getUri(), "rw")).getChannel();
            reader = ((FileInputStream) ctx.getContentResolver().openInputStream(af.docFile.getUri())).getChannel();
        }
        // Unable to write to writable file, can happen in Vista if have Create
        // Folders/Append Data set to Deny
        catch (IOException ioe)
        {
            ...
            catch (IOException ioe2)
            {
                // Warn but assume has worked okay
                logger.log(Level.WARNING, ErrorMessage.GENERAL_WRITE_PROBLEM_CLOSING_FILE_HANDLE
                        .getMsg(af.docFile, ioe.getMessage()), ioe2);
            }
        ...
        try
        {
            ...
            try
            {
                ...
                writeTag(af.getTag(), raf, reader, rafTemp);
                if (this.modificationListener != null)
                {
//                    this.modificationListener.fileModified(af, newFile);
                }
        ...
        {
            logger.log(Level.SEVERE, ErrorMessage.GENERAL_WRITE_FAILED_BECAUSE
                    .getMsg(af.docFile, e.getMessage()), e);
        ...
        if (newFile.length() > 0)
        {
            ...
            // Rename Original File
            // Can fail on Vista if have Special Permission 'Delete' set Deny
            DocumentFile originalFileBackup = af.docFile.getParentFile().createFile(
                    af.docFile.getType(),
                    AudioFile.getBaseFilename(af.docFile) + ".old"
            );
//            File originalFileBackup = new File(af.docFile.getParentFile().getPath(),
//                    AudioFile.getBaseFilename(af.getFile()) + ".old");
...
//            }
            ...
            boolean renameResult = Utils.rename(af.docFile, originalFileBackup);
            if (!renameResult)
            {
                logger
                        .log(Level.SEVERE, ErrorMessage.GENERAL_WRITE_FAILED_TO_RENAME_ORIGINAL_FILE_TO_BACKUP
                                .getMsg(af.docFile.getUri(), originalFileBackup.getName()));
                //Delete the temp file because write has failed
                if(newFile!=null)
                {
                    newFile.delete();
            ...
            // Rename Temp File to Original File
            renameResult = Utils.rename(newFile, af.docFile);
            if (!renameResult)
            {
                // Renamed failed so lets do some checks rename the backup back to the original file
                // New File doesnt exist
                ...
                // Rename the backup back to the original
                if (!originalFileBackup.renameTo(af.docFile.getName()))
                {
                    // TODO now if this happens we are left with testfile.old
                    // instead of testfile.mp4
                    logger
                            .warning(ErrorMessage.GENERAL_WRITE_FAILED_TO_RENAME_ORIGINAL_BACKUP_TO_ORIGINAL
                                    .getMsg(originalFileBackup.getUri(), af.getFile()
                                            .getName()));
                }
            ...
            else
            {
                ...
                if (!deleteResult)
                {
                    ...
                    logger
                            .warning(ErrorMessage.GENERAL_WRITE_WARNING_UNABLE_TO_DELETE_BACKUP_FILE
                                    .getMsg(originalFileBackup.getUri()));
                }
            }
...
//            this.modificationListener.fileOperationFinished(result);
    ...
    protected void deleteTag(FileChannel raf, FileChannel reader, FileChannel tempRaf) throws CannotWriteException, IOException {}
}
