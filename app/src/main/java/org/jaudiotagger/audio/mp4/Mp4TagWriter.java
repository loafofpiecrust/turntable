...
public class Mp4TagWriter
{
    ...
    public void write(Tag tag, FileChannel raf, FileChannel reader, FileChannel rafTemp) throws CannotWriteException, IOException
    {
        ...
        //Read Channel for reading from old file
        FileChannel fileReadChannel = reader;

        //Write channel for writing to new file
        FileChannel fileWriteChannel = rafTemp;
...
//                throw new CannotWriteException(ErrorMessage.MP4_CHANGES_TO_FILE_FAILED_INCORRECT_OFFSETS.getMsg(discrepancy));
    ...
    private void checkFileWrittenCorrectly(FileChannel rafTemp, Mp4BoxHeader mdatHeader, FileChannel fileWriteChannel, Mp4StcoBox stco)
            throws CannotWriteException,IOException
    {
...
//                throw new CannotWriteException(ErrorMessage.MP4_CHANGES_TO_FILE_FAILED_INCORRECT_OFFSETS.getMsg(discrepancy));
    ...
    public void delete(FileChannel raf, FileChannel reader, FileChannel rafTemp) throws IOException
    {
        Mp4Tag tag = new Mp4Tag();

        try
        {
            write(tag, raf, reader, rafTemp);
        }
        catch (CannotWriteException cwe)
        {
            throw new IOException(cwe.getMessage());
