...
public class Mp4BoxHeader
{
    ...
    public static Mp4BoxHeader seekWithinLevel(FileChannel input, String id) throws IOException
    {
        logger.finer("Started searching for:" + id + " in file at:" + input.position());
        ...
        while (!boxHeader.getId().equals(id))
        {
            logger.finer("Found:" + boxHeader.getId() + " Still searching for:" + id + " in file at:" + input.position());
            ...
            input.position(prev + boxHeader.getDataLength());
            long noOfBytesSkipped = input.position() - prev;
            logger.finer("Skipped:" + noOfBytesSkipped);
            if (noOfBytesSkipped < boxHeader.getDataLength())
            {
                return null;
            ...
            headerBuffer.rewind();
            bytesRead = input.read(headerBuffer);
            logger.finer("Header Bytes Read:" + bytesRead);
            headerBuffer.rewind();
            if (bytesRead == Mp4BoxHeader.HEADER_LENGTH)
            {
