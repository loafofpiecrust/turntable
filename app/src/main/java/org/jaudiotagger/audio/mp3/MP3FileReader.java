...
public class MP3FileReader extends AudioFileReader
{
    ...
    @Override public AudioFile read(File f) throws IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
    ...
    //Override because we read mp3s differently to the entagged code
    @Override public AudioFile read(Context ctx, DocumentFile f) throws IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
        MP3File mp3File = new MP3File(ctx, f, MP3File.LOAD_IDV1TAG | MP3File.LOAD_IDV2TAG, true);
        return mp3File;
    }
