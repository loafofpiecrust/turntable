...
public class MP3AudioHeader implements AudioHeader
{
    ...
    public MP3AudioHeader(final FileChannel seekFile, long startByte) throws IOException, InvalidAudioFrameException
    {
        if (!seek(seekFile, startByte))
        {
            throw new InvalidAudioFrameException(ErrorMessage.NO_AUDIO_HEADER_FOUND.getMsg(seekFile));
        }
    }
    ...
    public boolean seek(final FileChannel fc, long startByte) throws IOException
    {
        //References to Xing/VRbi Header
        ByteBuffer header;
...
//        final FileChannel fc = fis.getChannel();
        ...
        //Move FileChannel to the starting position (skipping over tag if any)
        System.out.println("tags: size = " + fc.size());
        fc.position(startByte);
//        bytesRead += fc.skip(startByte);
//        fc.mark(FILE_BUFFER_SIZE * 2);

        //Update filePointerCount
        filePointerCount = startByte;

        //Read from here into the byte buffer , doesn't move location of filepointer
//        fc.mark(FILE_BUFFER_SIZE);
        fc.read(bb, startByte);
//        fc.reset();
        bb.flip();
...
//                    fc.reset();
                ...
                if (MPEGFrameHeader.isMPEGFrame(bb))
                {
                    try
                    {
                        ...
                        {
                            syncFound = isNextFrameValid(fc, filePointerCount, bb);
                            if (syncFound)
                            {
                                break;
                            }
...
//            input.reset();
    ...
    private boolean isNextFrameValid(FileChannel fc, long filePointerCount, ByteBuffer bb) throws IOException
    {
        if (MP3AudioHeader.logger.isLoggable(Level.FINEST))
        {
//            MP3AudioHeader.logger.finer("Checking next frame" + seekFile.getName() + ":fpc:" + filePointerCount + "skipping to:" + (filePointerCount + mp3FrameHeader.getFrameLength()));
