...
public class AudioFile
{
    ...
    public AudioFile(DocumentFile f, AudioHeader audioHeader, Tag tag) {
        this.docFile = f;
        this.audioHeader = audioHeader;
        this.tag = tag;
    }
     ...
     * @see AudioFileIO
     */
    public void commit(Context ctx) throws CannotWriteException
    {
        AudioFileIO.write(ctx, this);
    }
    public void commit() throws CannotWriteException
    {
        AudioFileIO.write(null, this);
    }
     ...
     * artist, album, name, etc. If the file does not contain any tag the null is returned. Some audio formats do
    ...
    public void checkFileExists(DocumentFile file)throws FileNotFoundException
    {
        logger.config("Reading file:" + "path" + file.getUri());
        if (!file.exists())
        {
            logger.severe("Unable to find:" + file.getUri());
            throw new FileNotFoundException(ErrorMessage.UNABLE_TO_FIND_FILE.getMsg(file.getUri()));
        }
    }
    ...
    protected InputStream checkFilePermissions(Context ctx, DocumentFile file, boolean readOnly) throws ReadOnlyFileException, FileNotFoundException
    {
        if (file.canRead()) {
    ...
    public static String getBaseFilename(DocumentFile file)
    {
        int index = file.getName().toLowerCase().lastIndexOf(".");
        if(index > 0)
        {
