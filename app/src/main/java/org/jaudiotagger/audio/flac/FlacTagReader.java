...
public class FlacTagReader
{
    ...
    public FlacTag read(FileChannel raf) throws CannotReadException, IOException
    {
        FlacInputStreamReader flacStream = new FlacInputStreamReader(raf);
        flacStream.findStream();
        ...
        while (!isLastBlock)
        {
            if(logger.isLoggable(Level.CONFIG))
            {
                logger.config("Looking for MetaBlockHeader at:"+raf.position());
            }
            ...
            if(logger.isLoggable(Level.CONFIG))
            {
                logger.config("Reading MetadataBlockHeader:"+mbh.toString() + " ending at "+raf.position());
            }
            ...
            if(mbh.getBlockType()!=null)
            {
                switch (mbh.getBlockType())
                {
                    ...
                    case VORBIS_COMMENT:
                        byte[] commentHeaderRawPacket = new byte[mbh.getDataLength()];
                        raf.read(ByteBuffer.wrap(commentHeaderRawPacket));
                        tag = vorbisCommentReader.read(commentHeaderRawPacket, false);
                        break;
                    ...
                    default:
                        ...
                        raf.position(raf.position() + mbh.getDataLength());
                        break;
                }
            }
            isLastBlock = mbh.isLastBlock();
