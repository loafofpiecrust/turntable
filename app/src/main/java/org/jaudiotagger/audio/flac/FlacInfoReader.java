...
public class FlacInfoReader
{
    ...
    public FlacAudioHeader read(FileChannel input) throws CannotReadException, IOException
    {
        FlacInputStreamReader flacStream = new FlacInputStreamReader(input);
        flacStream.findStream();
        ...
        while (!isLastBlock)
        {
            MetadataBlockHeader mbh = MetadataBlockHeader.readHeader(input);
            if (mbh.getBlockType() == BlockType.STREAMINFO)
            {
                mbdsi = new MetadataBlockDataStreamInfo(mbh, input);
                if (!mbdsi.isValid())
                {
                    throw new CannotReadException("FLAC StreamInfo not valid");
                }
            ...
            {
                input.position(input.position() + mbh.getDataLength());
            }
        ...
        info.setEncodingType(mbdsi.getEncodingType());
        info.setExtraEncodingInfos("");
        info.setBitrate(computeBitrate(mbdsi.getPreciseLength(), input.size()));
        info.setLossless(true);
        info.setMd5(mbdsi.getMD5Signature());
        return info;
    }
