...
public class FlacTagWriter
{
    ...
    public void delete(FileChannel raf, FileChannel reader, FileChannel tempRaf) throws IOException, CannotWriteException
    {
        //This will save the file without any Comment or PictureData blocks
        FlacTag emptyTag = new FlacTag(null, new ArrayList<MetadataBlockDataPicture>());
        raf.position(0);
        tempRaf.position(0);
        reader.position(0);
        write(emptyTag, raf, reader, tempRaf);
    }
    ...
    public void write(Tag tag, FileChannel raf, FileChannel reader, FileChannel rafTemp) throws CannotWriteException, IOException
    {
        logger.config("Writing tag");
        ...
        //Read existing data
        FlacInputStreamReader flacStream = new FlacInputStreamReader(reader);
        try
        {
            flacStream.findStream();
        }
        ...
        while (!isLastBlock)
        {
            try
            {
                MetadataBlockHeader mbh = MetadataBlockHeader.readHeader(reader);
                if(mbh.getBlockType()!=null)
                {
                    switch (mbh.getBlockType())
                    {
                        case STREAMINFO:
                        {
                            blockInfo.streamInfoBlock = new MetadataBlock(mbh,new MetadataBlockDataStreamInfo(mbh, reader));
                            break;
                        }
                        ...
                        case PICTURE:
                        {
                            //All these will be replaced by the new metadata so we just treat as padding in order
                            //to determine how much space is already allocated in the file
                            reader.position(reader.position() + mbh.getDataLength());
                            MetadataBlockData mbd = new MetadataBlockDataPadding(mbh.getDataLength());
                            blockInfo.metadataBlockPadding.add(new MetadataBlock(mbh, mbd));
                            break;
                        }
                        case APPLICATION:
                        {
                            MetadataBlockData mbd = new MetadataBlockDataApplication(mbh, reader);
                            blockInfo.metadataBlockApplication.add(new MetadataBlock(mbh, mbd));
                            break;
                        }
                        case SEEKTABLE:
                        {
                            MetadataBlockData mbd = new MetadataBlockDataSeekTable(mbh, reader);
                            blockInfo.metadataBlockSeekTable.add(new MetadataBlock(mbh, mbd));
                            break;
                        }
                        case CUESHEET:
                        {
                            MetadataBlockData mbd = new MetadataBlockDataCueSheet(mbh, reader);
                            blockInfo.metadataBlockCueSheet.add(new MetadataBlock(mbh, mbd));
                            break;
                        }
                        default:
        ...
        //Go to start of Flac within file
        raf.position(flacStream.getStartOfFlacInFile());
        ...
        {
            //Jump over Id3 (if exists) Flac and StreamInfoBlock
            raf.position(flacStream.getStartOfFlacInFile() + FlacStreamReader.FLAC_STREAM_IDENTIFIER_LENGTH);

            //Write StreamInfo, we always write this first even if wasn't first in original spec
            raf.write(ByteBuffer.wrap(blockInfo.streamInfoBlock.getHeader().getBytesWithoutIsLastBlockFlag()));
            raf.write(ByteBuffer.wrap(blockInfo.streamInfoBlock.getData().getBytes()));

            //Write Application Blocks
            for (MetadataBlock aMetadataBlockApplication : blockInfo.metadataBlockApplication)
            {
                raf.write(ByteBuffer.wrap(aMetadataBlockApplication.getHeader().getBytesWithoutIsLastBlockFlag()));
                raf.write(ByteBuffer.wrap(aMetadataBlockApplication.getData().getBytes()));
            }

            //Write Seek Table Blocks
            for (MetadataBlock aMetadataBlockSeekTable : blockInfo.metadataBlockSeekTable)
            {
                raf.write(ByteBuffer.wrap(aMetadataBlockSeekTable.getHeader().getBytesWithoutIsLastBlockFlag()));
                raf.write(ByteBuffer.wrap(aMetadataBlockSeekTable.getData().getBytes()));
            }

            //Write Cue sheet Blocks
            for (MetadataBlock aMetadataBlockCueSheet : blockInfo.metadataBlockCueSheet)
            {
                raf.write(ByteBuffer.wrap(aMetadataBlockCueSheet.getHeader().getBytesWithoutIsLastBlockFlag()));
                raf.write(ByteBuffer.wrap(aMetadataBlockCueSheet.getData().getBytes()));
            }

            //Write tag (and padding)
            raf.write(tc.convert(tag, availableRoom - neededRoom));
        }
        //Need to move audio
        else
        {
            ...
            {
                raf.position(0);
                rafTemp.transferFrom(raf, 0, flacStream.getStartOfFlacInFile());
                rafTemp.position(flacStream.getStartOfFlacInFile());
            }
            rafTemp.write(ByteBuffer.wrap(FlacStreamReader.FLAC_STREAM_IDENTIFIER.getBytes()));
            rafTemp.write(ByteBuffer.wrap(new byte[] { 0 }));  //To ensure never set Last-metadata-block flag even if was before

            int uptoStreamHeaderSize = flacStream.getStartOfFlacInFile()
                    + FlacStreamReader.FLAC_STREAM_IDENTIFIER_LENGTH
                    + MetadataBlockHeader.BLOCK_TYPE_LENGTH;
            rafTemp.position(uptoStreamHeaderSize);
            raf.position(uptoStreamHeaderSize);

            rafTemp.transferFrom(
                    raf,
                    uptoStreamHeaderSize,
                    MetadataBlockHeader.BLOCK_LENGTH + MetadataBlockDataStreamInfo.STREAM_INFO_DATA_LENGTH);
            ...
            int dataStartSize = flacStream.getStartOfFlacInFile()
                    ...
                    + MetadataBlockHeader.HEADER_LENGTH
                    + MetadataBlockDataStreamInfo.STREAM_INFO_DATA_LENGTH;
            rafTemp.position(dataStartSize);

            //Write all the metadatablocks
            for (MetadataBlock aMetadataBlockApplication : blockInfo.metadataBlockApplication)
            {
                rafTemp.write(ByteBuffer.wrap(aMetadataBlockApplication.getHeader().getBytesWithoutIsLastBlockFlag()));
                rafTemp.write(ByteBuffer.wrap(aMetadataBlockApplication.getData().getBytes()));
            }

            for (MetadataBlock aMetadataBlockSeekTable : blockInfo.metadataBlockSeekTable)
            {
                rafTemp.write(ByteBuffer.wrap(aMetadataBlockSeekTable.getHeader().getBytesWithoutIsLastBlockFlag()));
                rafTemp.write(ByteBuffer.wrap(aMetadataBlockSeekTable.getData().getBytes()));
            }

            for (MetadataBlock aMetadataBlockCueSheet : blockInfo.metadataBlockCueSheet)
            {
                rafTemp.write(ByteBuffer.wrap(aMetadataBlockCueSheet.getHeader().getBytesWithoutIsLastBlockFlag()));
                rafTemp.write(ByteBuffer.wrap(aMetadataBlockCueSheet.getData().getBytes()));
            }

            //Write tag data use default padding
            rafTemp.write(tc.convert(tag, FlacTagCreator.DEFAULT_PADDING));
            //Write audio to new file
            raf.position(dataStartSize + availableRoom);

            //Issue #385
            //Transfer 'size' bytes from raf at its current position to rafTemp at position but do it in batches
            //to prevent OutOfMemory exceptions
            long amountToBeWritten=raf.size() - raf.position();
            long written   = 0;
            long chunksize = TagOptionSingleton.getInstance().getWriteChunkSize();
            long count = amountToBeWritten / chunksize;
            long mod   = amountToBeWritten % chunksize;
            for(int i = 0; i<count; i++)
            {
                written+=rafTemp.transferFrom(raf, rafTemp.position(), chunksize);
                rafTemp.position(rafTemp.position() + chunksize);
            }
            written+=rafTemp.transferFrom(raf, rafTemp.position(), mod);
            if(written!=amountToBeWritten)
            {
                throw new CannotWriteException("Was meant to write "+amountToBeWritten+" bytes but only written "+written+" bytes");
            }
