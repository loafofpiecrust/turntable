...
public abstract class AbstractID3v2Tag extends AbstractID3Tag implements Tag
{
    ...
    private static boolean isID3V2Header(FileChannel raf) throws IOException
    {
        long start = raf.position();
        byte[] tagIdentifier = new byte[FIELD_TAGID_LENGTH];
        raf.read(ByteBuffer.wrap(tagIdentifier));
        raf.position(start);
        if (!(Arrays.equals(tagIdentifier, TAG_ID)))
        {
            return false;
    ...
    public static boolean isId3Tag(FileChannel raf) throws IOException
    {
        if (!isID3V2Header(raf))
        {
            return false;
        ...
        //So we have a tag
        byte[] tagHeader = new byte[FIELD_TAG_SIZE_LENGTH];
        raf.position(raf.position() + 6);
        ByteBuffer bb = ByteBuffer.wrap(tagHeader);
        raf.read(bb);

        int size = ID3SyncSafeInteger.bufferToValue(bb);
        raf.position(size + TAG_HEADER_LENGTH);
        return true;
    }
